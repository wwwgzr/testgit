...
namespace CommonModule
{
    public class ConvertType
    {
        ...
        public static byte[] CopyToArray(byte[] data,ref int index, int len)
        {
            ...
            {
                WriteFileLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss ： ") + ex.Message + "CopyToArray\r\n");
                return null;
            }
        }
        /// <summary>
        ...
        public static void WriteFileError(string log)
        {
            try
            {
                string path = AppDomain.CurrentDomain.SetupInformation.ApplicationBase + "log";
                ...
                File.AppendAllText(path + "\\error" + DateTime.Now.ToString("yyyy-MM-dd") + ".txt", log);
            }
            catch (Exception ex)
            {
            }
        ...
        public static void DeleteLogFile()
        {
            ...
            foreach (string item in files)
            {
                try
                {
                    ...
                    if (dt1.Subtract(dt2).TotalDays > 120)
                    {
                        File.Delete(item);
                    }
                }
                ...
                {
                    WriteFileLog(ex.Message + "DeleteLogFile");
                    WriteFileLog(item + "删除失败 DeleteLogFile");
                }
            }
        }
