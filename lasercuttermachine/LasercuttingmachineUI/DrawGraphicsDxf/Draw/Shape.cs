...
namespace DrawGraphicsDxf.Draw
{
    public abstract class Shape
    {
        ...
        public virtual void DrawClone(Graphics g,float width,Pen p = null)
        {
            if (graphicsPath == null)
                return;
            if (pen != null && pen.Length > 0)
            {
                GraphicsPath gpclone = (GraphicsPath)graphicsPath.Clone();
                Matrix m = new Matrix();
                m.Translate(-width, 0);
                gpclone.Transform(m);
                //g.DrawPath(pen[0], gpclone);
                if(p != null)
                    g.DrawPath(p, gpclone);
                else
                    g.DrawPath(Pens.Yellow, gpclone);
            }
