...
namespace LasercuttingmachineUI
{
    ...
    public partial class PaintingPanel : UserControl
    {
        ...
        public List<Shape> dLineListClone = new List<Shape>();
        PointF pt2 = new PointF(100, 100);
        public float heightG;
        public float widthG;
        private string filePath1 = "";
        public float precent = 0.1f;
        Pen redPen = new Pen(Color.Red, 1);

        /// <summary>
        /// 是否是单头工作
        /// </summary>
        public static bool isSingleLaser = true;
        /// <summary>
        /// 是否是单头工作
        /// </summary>
        public static bool IsSingleLaser
        {
            get { return PaintingPanel.isSingleLaser; }
            set 
            {
                if (PaintingPanel.isSingleLaser != value)
                {
                    PaintingPanel.isSingleLaser = value; 
                }
            }
        }
        ...
        public string FilePath
        {
            ...
            { 
                ...
                if (filePath1 != "")
                {
                    ...
                    if (this.FindForm() == MenuController.owner)
                    {
                        MenuController.owner.info_Form.lbl_FileNameText = Path.GetFileName(filePath1);
                        MenuController.owner.info_Form.lbl_FileXText = widthG.ToString("f2");
                        MenuController.owner.info_Form.lbl_FileYText = heightG.ToString("f2");
                    }
                }
            }
        }
        ...
        private void GetDxfBound()
        {
            ...
            if (!IsSingleLaser)
            {
                PointF pt1 = new PointF(mouseDown.X + x * (1 + precent), mouseDown.Y + y * precent);
                PointF pt2 = new PointF(mouseDown.X - x * (1 + precent), mouseDown.Y - y * (1 + precent));
                mouseDown = pt1;
                mouseMove = pt2;
            }
            else
            {
                PointF pt1 = new PointF(mouseDown.X + x * precent, mouseDown.Y + y * precent);
                PointF pt2 = new PointF(mouseDown.X - x * (1 + precent), mouseDown.Y - y * (1 + precent));
                mouseDown = pt1;
                mouseMove = pt2;
            }
        ...
        private void Form1_Paint(object sender, PaintEventArgs e)
        {
            try
            {
                ...
                GraphicsPath gpBounds = new GraphicsPath();
                for (int i = 0; i < dLineList.Count; i++)
                {
                    ...
                    if (!IsSingleLaser)
                    {
                        RectangleF pathBound = dLineList[i].graphicsPath.GetBounds();
                        if (pathBound.Height > 0 && pathBound.Width > 0)
                        {
                            gpBounds.AddRectangle(pathBound);
                        }
                        else
                            gpBounds.AddPath(dLineList[i].graphicsPath, false);

                        gpBounds.StartFigure();
                    }
                }

                float w = gpBounds.GetBounds().Width;
                ...
                if (!IsSingleLaser)
                {
                    ...
                    {
                        if(isView)
                            dLineList[i].DrawClone(g, w,redPen);
                        else
                            dLineList[i].DrawClone(g, w);
                    }
                }
            ...
            {
                CommonModule.ConvertType.WriteFileLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss ： ") + ex.Message + "Form1_Paint\r\n");
            }
        }
        ...
        public string ConvertToPLT(bool isWrite = false,bool isNeedBreak = false)
        {
            //isNeedBreak = true;
            if (FilePath == "")
                return "";
            ...
            for (int j = 0; j < orderPoints.Length; j++)
            {
                ...
                for (int i = 0; i < ptList.Length; i++)
                {
                    ...
                    {
                        if (isNeedBreak && i - 1 > 0 && ptList[i].Y == 2 && ptList[i - 1].Y == 2)
                        {
                            //isBreakLine = true;
                            if (pDText.Length > 2)
                            {
                                str += pDText.Substring(0,pDText.Length - 1) + ";";
                            }
                            if (!isCloseB)
                            {
                                str += "CL2;";
                                //str += "SP2;";
                                str += "PU" + ftoi(ptList[i - 1].X) + "," + ftoi((ptList[i - 1].Y)) + ";";
                                pDBreakLine += ftoi(ptList[i].X) + "," + ftoi((ptList[i].Y)) + ";";
                                isBreakLine = true;
                            }
                            isCloseB = true;

                            if (i + 1 < ptList.Length && ptList[i].Y == miny && ptList[i + 1].Y == miny)
                            {
                                if(!isBreakLine)
                                    pDBreakLine += ftoi(ptList[i].X) + "," + ftoi((ptList[i].Y)) + ",";
                                isBreakLine = false;
        ...
        private void GetImageRect(out float minx,out float miny)
        {
            #region 求负值坐标
            RectangleF rect1 = GetPathBounds();
            minx = Math.Min((-rect1.Left), (-rect1.Right)) - 2;
            if (minx <= (int)minx)
                minx = (int)minx;
            miny = Math.Min((-rect1.Top), (-rect1.Bottom)) - 2;
            if (miny <= (int)miny)
                miny = (int)miny;
            #endregion 求负值坐标
        ...
        public void GetGraphicsPathPlt(PointT[] pointList)
        {
            ...
            {
                CommonModule.ConvertType.WriteFileLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss ： ") + ex.Message + "GetGraphicsPathPlt\r\n");
        ...
        public bool ReadPltFile(string filepath)
        {
            ...
            {
                CommonModule.ConvertType.WriteFileLog(DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss ： ") + ex.Message + "ReadPltFile\r\n");
                FilePath = "";
                return false;
            }
        ...
        public void DrawMovePath(Graphics g,float w)
        {
            if (dLineList.Count > 0 && pathIndex >= 0 && pointIndex >= 0 && movePath != null)
            {
                ...
                if(!IsSingleLaser)
                {
                    GraphicsPath gpclone = (GraphicsPath)movePath.Clone();
                    Matrix m1 = new Matrix();
                    m1.Translate(-w, 0);
                    gpclone.Transform(m1);
                    g.DrawPath(Pens.LightPink, gpclone);
                }
            }
        }
