...
namespace LasercuttingmachineUI
{
    ...
    public partial class PaintingPanel : UserControl
    {
        ...
        public string ConvertToPLT(bool isWrite = false)
        {
            ...
            #region 求负值坐标
            RectangleF rect1 = GetPathBounds();
            float minx = Math.Min((-rect1.Left),(-rect1.Right))-1;
            if (minx <= (int)minx)
                minx = (int)minx;
            float miny = Math.Min((-rect1.Top), (-rect1.Bottom))-1;
            if (miny <= (int)miny)
                miny = (int)miny;
            #endregion 求负值坐标
        ...
        public string GetAllPathPoints()
        {
            string allPointsStr = "";
            foreach (Shape item in dLineList)
            {
        ...
        //                    //Console.WriteLine("pathIndex:" + pathIndex + ",pointIndex:" + pointIndex);
        //                    if (r == 1)
        //                    {
        ...
        //    GraphicsPath gp = new GraphicsPath();
        //    //if (p.X == pt[pt.Length - 1].X && p.Y == pt[pt.Length - 1].Y && pointIndex != 0 && pointIndex + 3> pt.Length)
        //    //{
        //    //    pathIndex++;
        //    //    pointIndex = 0;
        //    //}
        ...
        //            //if (GraphicsMath.isPointOnSegment(pt[i], pt[i + 1], p))
        ...
        //        btype[i] = dLineList[pathIndex].graphicsPath.PathTypes[i];
        //    }
        //    //dLineList[pathIndex].graphicsPath.PathPoints
        //    movePath = new GraphicsPath();
        //    if (pointIndex == 0)
        //    {
        //        movePath = new GraphicsPath();
        ...
        /// </summary>
        /// <returns></returns>
        public PointCut[] GetPathStartEndPoint()
        {
            if (dLineList == null && dLineList.Count == 0)
                return null;
        ...
        /// </summary>
        /// <returns></returns>
        private PointCut[] GetOptimizePath()
        {
            PointCut[] pointCutArray = GetPathStartEndPoint();
            if (Path.GetExtension(FilePath).ToLower() == ".dxf")
            {
